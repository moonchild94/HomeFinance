...
interface OperationDao {
    ...
    @Query("SELECT Category.*, SUM(IdleOperation.sumMainamount) amount from IdleOperation INNER JOIN Category ON IdleOperation.categoryId = Category.categoryId WHERE IdleOperation.periodic != 1 and IdleOperation.walletId = :walletId and IdleOperation.date <= :dateTo and IdleOperation.date >= :dateFrom and Category.operationType = :operationType GROUP BY Category.categoryId")
    fun getOperationsSummaryByCategories(walletId: Int, dateFrom: Date, dateTo: Date, operationType: OperationType): Flowable<List<OperationStatistics>>
}